...
function DisplayContainer(props) {
  ...
  initWeather.current = async () => {
    if (isImperial) {
      const weatherCall = await axios
        .get(`/api/forecast/getForecast?lat=${latitude}&lon=${longitude}`, {
        ...
        .then(
          ({
            ...
            setNextSeven(
              initializeWeatherObjectArray(ResponseData.daily.slice(1, 7))
            );
          }
        )
        .catch((err) => {
  ...
  return props.mobileLandscape ? (
    currentDay ? (
      ...
      >
        ...
        >
          <HStack mr={10} alignSelf="flex-end">
          ...
          <Flex flexDirection="row">
            <Box>
              <GreetingCard
                city={currentDay ? currentDay.city : "Loading..."}
              />
              <WeatherCard
                isImperial={isImperial}
                weatherDesc={
                  currentDay != null ? currentDay.weather.main : "Loading"
                }
            ...
            <Box marginTop="12ex">
              <WeatherCardArray
                isImperial={isImperial}
                sourceArray={nextSeven}
                isLandscapeMode={mobileLandscape}
              />
            </Box>
          </Flex>
        </Flex>
